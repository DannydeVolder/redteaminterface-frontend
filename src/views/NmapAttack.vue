<template>
  <v-container fluid>
    ...
    <v-card style="color: #c7c7c7; width: 95%; margin-top: 60px; margin-left: 2.5%; margin-right: 2.5%;">
            ...
            <v-card-title style="font-size: 1.4em;font-weight: bold" class="justify-center" >Attack 1: Port scan (Nmap)</v-card-title>

            <v-card-text style="color: #c7c7c7; font-size: 1.2em;">
                <p>This is the first attack to be executed. This attack performs a port scan with Nmap, the results of which will show up on the SIEM dashboard.</p>
                <p>When the "Execute Attack" button is clicked, the attack will continue to execute once every 5 minutes until the "Stop Attack" button is clicked.</p>
...
export default {
    ...
    methods:{
        ...
        executeAttack(){
            try{
                API.executePortScan().then(response =>{
                    if(response.status === 200){        
                        this.attackExecuting = true;              
                        this.$store.dispatch("alert/success", 'Attack is executing.')
                    }
                    else{
